...
public class AnalyticsService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            // Get max ID for generating new ID
            var maxId = await _dbContext.AnalyticsMarketFeatures.MaxAsync(f => (ulong?)f.FeatureId, cancellationToken) ?? 0;
            feature.FeatureId = maxId + 1;
