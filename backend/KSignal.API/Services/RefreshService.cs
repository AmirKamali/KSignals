...
//             try
//             {
//                 await InsertMarketSnapshotsAsync(results, cancellationToken);
//             }
//             catch (Exception ex)
//             {
//                 _logger.LogError(ex, "Failed to insert {Count} today market snapshots for next {Days} day(s)", results.Count, safeDays);
//             }
//         }
...
//         // Remove MarketSnapshot records where CloseTime is more than 7 days ago
//         var oldMarkets = await db.MarketSnapshots
//             .Where(m => m.CloseTime <= sevenDaysAgo)
//             .ToListAsync(cancellationToken);

//         if (oldMarkets.Count > 0)
//         {
//             _logger.LogInformation("Removing {Count} MarketSnapshot records with CloseTime older than 7 days", oldMarkets.Count);
//             db.MarketSnapshots.RemoveRange(oldMarkets);
//             await db.SaveChangesAsync(cancellationToken);
//         }
//     }
...
//         // Insert market snapshots to database
//         if (markets.Count > 0)
//         {
//             await InsertMarketSnapshotsAsync(markets, cancellationToken);
//         }

//         return markets.Count;
//     }

//     private async Task InsertMarketSnapshotsAsync(List<MarketSnapshot> markets, CancellationToken cancellationToken)
//     {
//         using var scope = _scopeFactory.CreateScope();
//         var db = scope.ServiceProvider.GetRequiredService<KalshiDbContext>();
...
//         foreach (var batch in markets.Chunk(batchSize))
//         {
//             await db.MarketSnapshots.AddRangeAsync(batch, cancellationToken);
//         }
//         await db.SaveChangesAsync(cancellationToken);
//     }
